...
" Implementation functions {{{
  ...
  function! s:EchoBufferIndicators()
    ...
    call EchoWithColor('u  an unlisted buffer', 'Normal')
    call EchoWithColor('%  the buffer in the current window', 'Normal')
    call EchoWithColor('#  the alternate buffer', 'Normal')
    call EchoWithColor('a  an active buffer: it is loaded and visible', 'Normal')
    call EchoWithColor('h  a hidden buffer: It is loaded, but currently not', 'Normal')
    call EchoWithColor('-  a buffer with "modifiable" off', 'Normal')
    call EchoWithColor('=  a readonly buffer', 'Normal')
    call EchoWithColor('+  a modified buffer', 'Normal')
